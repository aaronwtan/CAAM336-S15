...
% method in space, and the Backward Euler method in time.
...
% initial value of coefficients in u_n(x,t) = sum alf_k(t) phi_k(x)
 ...
 for m=1:length(tvec)
    ...
    if m==1, Ue = U0;
    else,    Ue = (eye(2*n)-dt*A)\Ue;    % backward Euler
    end
    UU(m,:) = [0 U(1:n)' 0];
    UUe(m,:) = [0 Ue(1:n)' 0];
    subplot(3,1,1:2)
    ...
    ylabel('Backward Euler error','fontsize',16)
    xlabel('x','fontsize',16)
    drawnow
 ...
 title('Backward Euler', 'fontsize', 16)
 ...
 zlabel('u(x,t)','fontsize',16)
 ylim([min(tvec) max(tvec)])
 title('Backward Euler Error', 'fontsize', 16)
