...
% method in space, and the Trapezoid Rule in time.
...
% initial value of coefficients in u_n(x,t) = sum alf_k(t) phi_k(x)
 ...
 for m=1:length(tvec)
    ...
    if m==1, Ue = U0;
    else,    Ue = (eye(2*n)-.5*dt*A)\((eye(2*n)+.5*dt*A)*Ue);  % trapezoid
    end
    UU(m,:) = [0 U(1:n)' 0];
    UUe(m,:) = [0 Ue(1:n)' 0];
    subplot(3,1,1:2)
    ...
    ylabel('Trapezoid Rule error','fontsize',16)
    xlabel('x','fontsize',16)
    drawnow
 ...
 title('Trapezoid Rule', 'fontsize', 16)
 ...
 zlabel('u(x,t)','fontsize',16)
 ylim([min(tvec) max(tvec)])
 title('Trapezoid Rule Error', 'fontsize', 16)
